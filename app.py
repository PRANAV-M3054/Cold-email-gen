...
from llm_engine import genereate_cold_email_and_cover
...
resume_file =st.file_uploder("Upload your Resume (PDF/DOCX)",type=["pdf","docx"])
...
if send_option:
    recruiter_email = st.text_input("Recuiter's Email")
...
if st.button("Generate Email & Cover Letter"):
    if not resume_file or not job_url:
        ...
        with st.spinner("Processing..."):
            ...
            cold_email,cover_letter = genereate_cold_email_and_cover(
                resume_text, job_description, github_link, portfolio_link
            )
            
            st.subheader("Cold Email")
            st.text_area("Ggenerated Email", cold_email, heigth =250)
            
            st.subheader("Cover Letter")
            st.text_area("Genereated Cover Letter", cover_letter, heigth =350)
